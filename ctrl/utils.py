...
def plot_model(ctrl, D, rep_buf=10, H=None, L=10, fname=None, verbose=False, wandb_run=None):
    with torch.no_grad():
        if verbose:
            print(f"Plotting training sequences for model {ctrl.name}")
        ...
        st_hat, rt_hat, at_hat, t = ctrl.forward_simulate(
            tobs, st[:, 0, :], g, L=L, compute_rew=False
        )
        if verbose:
            print(st_hat.shape)
        plot_sequences(
            ctrl, fname, tobs, st, at, rt, t, st_hat, at_hat, rt_hat,
            verbose=verbose, wandb_run=wandb_run
        )

def plot_test(ctrl, D, N=None, H=None, L=10, fname=None, verbose=False, wandb_run=None):
    with torch.no_grad():
        if verbose:
            print(f"Plotting test sequences for model {ctrl.name}")
        H = D.H if H is None else H
        ...
        st_hat, rt_hat, at_hat, t = ctrl.forward_simulate(
            H, st[:, 0, :], g, L=L, compute_rew=False
        )
        if verbose:
            print(st_hat.shape)
        plot_sequences(
...
def plot_sequences(ctrl, fname, tobs, st, at, rt, t, st_hat, at_hat, rt_hat, verbose=False, wandb_run=None):
    plt.close()
    ...
    for j in range(N):
        for i in range(ctrl.env.n):
            ...
            plt.plot(t[j], st_hat[:, j, :, i].T, '-b', linewidth=0.75)
            if i == 0:
                plt.ylabel(f'Seq. {j + 1}', fontsize=20)
        ...
        for i in range(ctrl.env.m):
            ...
            plt.plot(t_acts[j], act_hats[:, j, :, i].T, '-b', linewidth=0.75)
            if j == 0:
    ...
    if wandb_run is not None:
        wandb_run.log({"plot": wandb.Image(plt.gcf())})
    plt.close()
...
def get_train_functions(dynamics, eta_dynamics):
    if 'ode' in dynamics:
        return partial(train_ode, eta_dynamics=eta_dynamics)
    elif 'pilco' in dynamics:
        return train_deep_pilco
    elif 'pets' in dynamics:
        return train_pets

def train_loop(ctrl, D, fname, Nround, wandb_run=None,eta_dynamics=1e-3, eta_policy=1e-3,**kwargs):
    H, L  = kwargs['H'], kwargs['L']
    verbose, print_times, N_pol_iter, Nexpseq = True, 10, 250, ctrl.env.Nexpseq
    dyn_tr_fnc = get_train_functions(ctrl.dynamics, eta_dynamics=eta_dynamics) # confirm the dynamics training function
    ...
    for round in range(r0,r0+Nround):
        ...
        # if round % 2 == 0:  # only do dynamics and policy learning on even rounds, i.e. 0,2,4,...
            # # dynamics training
            # dyn_tr_fnc(ctrl, D, fname, verbose, print_times, wandb_run=wandb_run, **kwargs)
            # # policy learning
            # train_policy(ctrl, D, H=H, V_const=min(round / 5.0, 1), verbose=verbose, \
            #              Niter=N_pol_iter, L=L, save_fname=fname, print_times=print_times, wandb_run=wandb_run)
        # dynamics training
        dyn_tr_fnc(ctrl, D, fname, verbose, print_times, wandb_run=wandb_run, **kwargs)
        # policy learning
        train_policy(ctrl, D, H=H, V_const=min(round / 5.0, 1), verbose=verbose, \
                     Niter=N_pol_iter, L=L, save_fname=fname, print_times=print_times, wandb_run=wandb_run, eta=eta_policy)
        # test the policy
        ...
        # Calculate imagined mean action
        mean_act = torch.stack(list(at_hat.values())).abs().mean()
        print('Imagined mean action is {:.2f}'.format(mean_act))

        # Log true/imagined reward sum, minimum test reward, and mean action to wandb
        wandb_run.log({
            "round": round,
            "true_test_reward_sum": true_test_rewards,
            "imagined_test_reward_sum": imagined_test_rewards,
            "min_test_reward": test_rewards[..., Tup:].min().item(),
            "imagined_mean_action": mean_act,
        ...
        if mean_act<0.25: # if the policy does not explore, collect data with random policy
            ...
            ######################CHANGE FOR OTHER ENVS!!!######################
            
            D = collect_experience(ctrl, D=D, N=1, H=D.dt*D.T, reset=True)
        plot_model(ctrl, D, H=H, L=10, rep_buf=10, fname=fname+'-train.png', verbose=False, wandb_run=wandb_run)
        ctrl.save(D=D, fname=fname)
    metric =wandb_run.history
    import pandas as pd
...
def train_policy(ctrl, D, H=2.0, Niter=250, verbose=True, tau=5.0, N=100, L=10, V_const=1.0, save_every=50, 
            eta=1e-3, save_fname=None, rep_buf=5, opt='adam', print_times=10, wandb_run=None):#eta policy
    if rep_buf<0:
        rep_buf = D.N
    if verbose: 
    ...
    if wandb_run:
        wandb_run.log({
            "policy_learning_opt_target": np.mean(opt_objs),
            "policy_learning_mean_reward": np.mean(rewards),
            "policy_learning_grad_norm": grad_norm,
            "policy_learning_first_value_error": first_val_err,
            "policy_learning_final_value_error": td_error.item(),
            "policy_learning_kl": ctrl.V.kl().item(),
        })
...
def train_dynamics(ctrl, D, Niter=1250, verbose=True, H=10, N=-1, L=1, eta=1e-3, eta_final=1e-4, \
        save_every=50, save_fname=None, func_KL=False, lr_sch=False, kl_w=1, rep_buf=-1, temp_opt=True, \
        num_plots=0, opt='adam', print_times=10, rnode=False, stop_mse=1e-3, nrep=3, wandb_run=None):
    if save_fname is None:
        save_fname = ctrl.name
    ...
    if lr_sch:
        scheduler = torch.optim.lr_scheduler.LambdaLR(
            optimizer.opt,  # Use the wrapped optimizer here
            lr_lambda=lambda epoch: (eta_final + 0.5 * (eta - eta_final) * \
                                     (1 + math.cos(math.pi * min(epoch / 100, 1)))) / eta
        )
    ...
    for k in range(Niter):
        ...
        if num_plots>0 and (k+1)%(Niter//num_plots) == 0:
            plot_model(ctrl, D, L=10, rep_buf=10, fname=save_fname+'-train.png', verbose=False, wandb_run=wandb_run)
            plot_test(ctrl,  D, L=10, N=10, fname=save_fname+'-test.png', verbose=False, wandb_run=wandb_run)
    ...
    if wandb_run:
        wandb_run.log({
            "dynamics_learning_loss": np.mean(losses),
            "dynamics_learning_mse": np.mean(mses),
            "dynamics_learning_lhood": np.mean(lhoods),
            "dynamics_learning_kl": np.mean(kls),
            "dynamics_learning_grad_norm": np.mean(grad_norms),
        })

    if num_plots>0:
        plot_model(ctrl, D, L=10, rep_buf=10, fname=save_fname+'-train.png', wandb_run=wandb_run)
        plot_test(ctrl,  D, L=10, N=10, fname=save_fname+'-test.png', wandb_run=wandb_run)
...
def train_ode(ctrl, D, save_fname, verbose, print_times, wandb_run=None, eta_dynamics=1e-3,**kwargs):
    ...
    for H_ in [ctrl.env.dt*5]:
        train_dynamics(ctrl, D, Niter=1250, L=kwargs['L'], H=H_, eta=eta_dynamics, eta_final=1e-4, save_fname=save_fname, \
            lr_sch= True, verbose=verbose, print_times=print_times, rep_buf=50, nrep=3, wandb_run=wandb_run)
