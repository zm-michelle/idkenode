...
def train_loop(ctrl, D, fname, Nround, wandb_run=None, **kwargs):
    H, L  = kwargs['H'], kwargs['L']
    verbose, print_times, N_pol_iter, Nexpseq = True, 10, 250, ctrl.env.Nexpseq
    dyn_tr_fnc = get_train_functions(ctrl.dynamics) # confirm the dynamics training function
    ...
    for round in range(r0,r0+Nround):
        ...
        if round % 3 == 0:  # only do dynamics and policy learning on even rounds, i.e. 0,3,6,...
            # dynamics training
        ...
        if round % 3 == 0:
            if mean_act < 0.25:  # if the policy does not explore, collect data with random policy
                D = collect_data(ctrl.env, H=D.dt * D.T, N=N_update, D=D)
            else:
                ######################CHANGE FOR OTHER ENVS!!!######################
                if Nexpseq != 1:
                    D = collect_experience(ctrl, D=D, N=N_update - 1, H=D.dt * D.T, reset=False,
                                           explore=True)
        ...
        # else:
        #     ######################CHANGE FOR OTHER ENVS!!!######################
        #     # D = collect_experience(ctrl, D=D, N=Nexpseq+1, H=D.dt*D.T, reset=False, explore=True) # Set N=Nexpseq+1 for pendulum
        #     D = collect_experience(ctrl, D=D, N=1, H=D.dt*D.T, reset=True)
...
def train_policy(ctrl, D, H=2.0, Niter=250, verbose=True, tau=5.0, N=100, L=10, V_const=1.0, save_every=50, 
            eta=1e-3, save_fname=None, rep_buf=5, opt='adam', print_times=10, wandb_run=None):
    if rep_buf<0:
        rep_buf = D.N
    if verbose: 
...
def train_ode(ctrl, D, save_fname, verbose, print_times, wandb_run=None, **kwargs):
    ...
    for H_ in [ctrl.env.dt*5]:
        train_dynamics(ctrl, D, Niter=1250, L=kwargs['L'], H=H_, eta=1e-3, eta_final=1e-4, save_fname=save_fname, \
...
def collect_experience(ctrl, D, N=1, H=None, reset=False, explore=False, sf=0.1):
    with torch.no_grad():
        ...
        # print(f"Inputs: D={D}, N={N}, H={H}, reset={reset}, explore={explore}")
...
def get_ivs(env, D, N, rep_buf=None):
    ...
    # print("_____s0_____:", s0)
    # print("_____N_____:", N)
    # print("_____idx_____:", idx)
    if N != 0:
